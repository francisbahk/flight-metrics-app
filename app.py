        ...
        if has_return:
            ...
            with col_ranking_out:
                ...
                if st.session_state.selected_flights:
                    ...
                    # Create flight labels with full metrics for drag-to-rank
                    flight_labels = []
                    for i, flight in enumerate(st.session_state.selected_flights):
                        ...
                        # Create multi-line label with all metrics
                        label = f"""#{i+1}: ${flight['price']:.0f} • {duration_display} • {stops_text}
...
{flight['origin']} → {flight['destination']} | {dept_date_display}"""
                    ...
                    # Sortable list with drag-to-rank
                    ...
                    # Add X buttons to remove flights
                    st.markdown("---")
                    st.markdown("**Remove flights:**")
                    cols = st.columns(5)
                    for i, flight in enumerate(st.session_state.selected_flights):
                        with cols[i]:

    ...
    if all_submitted:
        ...
        if st.session_state.get('search_id'):
            ...
            if not st.session_state.get('lilo_completed'):
                ...
                # LILO Initial Questions (generated by optimizer)
                if st.session_state.lilo_round == -1:
                    ...
                    # Generate initial questions using LILO optimizer
                    if 'lilo_initial_questions' not in st.session_state:
                        try:
                            # Get initial questions from optimizer
                            initial_questions = st.session_state.lilo_optimizer.generate_initial_questions()
                            st.session_state.lilo_initial_questions = initial_questions
                        except Exception as e:
                            st.error(f"Error generating questions: {e}")
                            # Fallback to simple questions
                            st.session_state.lilo_initial_questions = [
                                "What matters most to you when choosing a flight?",
                                "Are there any specific preferences or constraints we should know about?"
                            ]

                    questions = st.session_state.lilo_initial_questions
